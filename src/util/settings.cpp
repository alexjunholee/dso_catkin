...
/* Parameters for akaze */

//float kazeoptionssoffset = 0.5;
//float kazeoptionsnsublevels = 1;
//float kazeoptionsdthreshold = 2e-3;
//float kazeoptionsdescriptor_pattern_size = 9;
//float kazeoptionsimg_width = 240;
...
float setting_desiredImmatureDensity = 2000; // immature points per frame
float setting_desiredPointDensity = 2000; // aimed total points in the active window.
float setting_minPointsRemaining = 0.01;  // marg a frame if less than X% points remain.
...
int   setting_minFrames = 3; // min frames in window.
int   setting_maxFrames = 15; // max frames in window.
int   setting_minFrameAge = 1;
int   setting_maxOptIterations=10; // max GN iterations.
...
float setting_outlierTH = 2000*2000;					// higher -> less strict
float setting_outlierTHSumComponent = 10000*10000; 		// higher -> less strong gradient-based reweighting .




int setting_pattern = 8;						// point pattern used. DISABLED.
float setting_margWeightFac = 1;          // factor on hessian when marginalizing, to account for inaccurate linearization points.


/* when to re-track a frame */
float setting_reTrackThreshold = 2; // (larger = re-track more often)



/* require some minimum number of residuals for a point to become valid */
int   setting_minGoodActiveResForMarg=2;
int   setting_minGoodResForMarg=2;
...
float setting_huberTH = 2000; // Huber Threshold
...
float setting_maxPixSearch = 0.08; // max length of the ep. line segment searched during immature point tracking. relative to image resolution.
float setting_minTraceQuality = 2;
int setting_minTraceTestRadius = 1;
int setting_GNItsOnPointActivation = 3;
float setting_trace_stepsize = 1.0;				// stepsize for initial discrete search.
int setting_trace_GNIterations = 10;				// max # GN iterations
float setting_trace_GNThreshold = 0.05;				// GN stop after this stepsize.
float setting_trace_extraSlackOnTH = 1;			// for energy-based outlier check, be slightly more relaxed by this factor.
float setting_trace_slackInterval = 2;			// if pixel-interval is smaller than this, leave it be.
float setting_trace_minImprovementFactor = 10;		// if pixel-interval is smaller than this, leave it be.
...
float benchmark_varBlurNoise = 1;
float benchmark_initializerSlackFactor = 1;
int benchmark_noiseGridsize = 10;
