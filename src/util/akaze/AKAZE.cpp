...
int AKAZE::fill_variables_for_matching(cv::Mat img)
{
  ...
  for (size_t i = 1; i < evolution_.size(); i++) {
    ...
    gaussian_2D_convolution(evolution_[i].Lt, evolution_[i].Lsmooth, 0, 0, 2.0);
...
void AKAZE::Find_Scale_Space_Extrema(std::vector<cv::KeyPoint*> kpts) {
  ...
  cv::KeyPoint* point;
  std::vector<cv::KeyPoint*> kpts_aux;
...
void AKAZE::Compute_Descriptors(Eigen::Vector3f* dIp[3], std::vector<cv::KeyPoint*> kpts, std::vector<std::vector<bool>*>* desc) {
...
#pragma omp parallel for
  ...
  {
    std::vector<bool> descnow = {};
//    Compute_Main_Orientation(kpts[i]);
    Get_MLDB_Full_Descriptor_vec(dIp, kpts[i], &descnow);
    desc->push_back(&descnow);
  }
...
/* ************************************************************************* */
void AKAZE::Get_MLDB_Full_Descriptor_vec(Eigen::Vector3f* dIp[3], cv::KeyPoint* kpt, std::vector<bool>* desc) {
  ...
  for(int lvl = 0; lvl < 3; lvl++) {
    ...
    MLDB_Fill_Values_img(dIp[lvl], values, sample_step, kpt->class_id, xf, yf, si, co, scale);
    MLDB_Binary_Comparisons_vec(values, &descnow, val_count, dpos);
  }
  *desc = descnow;
}
void AKAZE::MLDB_Binary_Comparisons_vec(float* values, std::vector<bool>* desc,
                                    int count, int& dpos) {
  ...
  for(int pos = 0; pos < nr_channels; pos++) {
    for (int i = 0; i < count; i++) {
      ...
      for (int j = i + 1; j < count; j++) {
        bool res = ival > values[nr_channels * j + pos];
        desc->push_back(res);
        dpos++;
      }
    }
...
/* ************************************************************************* */
void AKAZE::Compute_Main_Orientation(cv::KeyPoint* kpt) {
...
/* ************************************************************************* */
void AKAZE::MLDB_compute_residual(const Eigen::Vector3f* const mat, float* residual, std::vector<bool> desc, float* values,
                                  int sample_step, int level, float xf, float yf, float scale, const int width, float angle, int* dpos, int count) {
  xf /= pow(2,level);
  yf /= pow(2,level);
  int ix = (int)xf;
  int iy = (int)yf;
    ...
    for (int j = -pattern_size; j < pattern_size; j += sample_step) {
      ...
      for (int k = i; k < i + sample_step; k++) {
        for (int l = j; l < j + sample_step; l++) {
          ...
          Eigen::Vector3f rixy = wxwy * (*(const Eigen::Vector3f*)(mat+x1+y1*width + 1+width))
                          + (wy-wxwy) * (*(const Eigen::Vector3f*)(mat+x1+y1*width + width))
                          + (wx-wxwy) * (*(const Eigen::Vector3f*)(mat+x1+y1*width + 1))
                     + (1-wx-wy+wxwy) * (*(const Eigen::Vector3f*)(mat+x1+y1*width));

          float ri = rixy[0]; float rx = rixy[1]; float ry = rixy[2];
  ...
  for(int pos = 0; pos < nr_channels; pos++) {
    for (int i = 0; i < count; i++) {
      float ival = values[nr_channels * i + pos];
      for (int j = i + 1; j < count; j++) {
        float res = ival - values[nr_channels * j + pos];
        unsigned long descpos = *dpos;
        if (desc[descpos]) *residual = res > 0 ? *residual : *residual+1;
        else *residual = res < 0 ? *residual :  *residual+1;
        *dpos = (int) descpos + 1;
      }
    }
  }
...
/* ************************************************************************* */
void AKAZE::MLDB_Fill_Values_img(Eigen::Vector3f* dI, float* values, int sample_step, int level,
                             float xf, float yf, float co, float si, float scale) {

  xf /= pow(2,level);
  yf /= pow(2,level);
  const int width = 240 / pow(2,level);
  int ix = (int)xf;
  int iy = (int)yf;
  float dx = xf - ix;
  float dy = yf - iy;
    ...
    for (int j = -pattern_size; j < pattern_size; j += sample_step) {
      ...
      for (int k = i; k < i + sample_step; k++) {
        for (int l = j; l < j + sample_step; l++) {
          ...
          Eigen::Vector3f rixy = dxdy * (*(const Eigen::Vector3f*)(dI+x1+y1*width + 1+width))
                          + (dy-dxdy) * (*(const Eigen::Vector3f*)(dI+x1+y1*width + width))
                          + (dx-dxdy) * (*(const Eigen::Vector3f*)(dI+x1+y1*width + 1))
                     + (1-dx-dy+dxdy) * (*(const Eigen::Vector3f*)(dI+x1+y1*width));
