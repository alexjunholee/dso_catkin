...
namespace libAKAZE {
  ...
  public:
    ...
    void Compute_Descriptors(Eigen::Vector3f* dIp[3], std::vector<cv::KeyPoint*> kpts, std::vector<std::vector<bool>*>* desc);
    ...
    void Get_MLDB_Full_Descriptor_vec(Eigen::Vector3f* dIp[3], cv::KeyPoint* kpt, std::vector<bool>* desc);
    ...
    void MLDB_Fill_Values_img(Eigen::Vector3f* dI, float* values, int sample_step, int level,
    ...
    void MLDB_Binary_Comparisons_vec(float* values, std::vector<bool>* desc, int count, int& dpos);

    /// Compute residual for optimization.
    void MLDB_compute_residual(const Eigen::Vector3f* const mat, float* residual, std::vector<bool> desc, float* values,
